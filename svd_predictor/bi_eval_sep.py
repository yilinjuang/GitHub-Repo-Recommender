...
for user_index, repo_index in zip(nonzero_user_indices, nonzero_repo_indices):
    nonzero_positions.add((user_index, repo_index))
with open("{}.edge".format(sys.argv[3]), "rb") as f:
    target_edges = pickle.load(f)
    target_edges = set(target_edges)
    N_PREDICT = len(target_edges)   # Total edges to predict.

# Perform singular value decomposition.
# K = 10     # Number of eigenvectors/eigenvalues of SVD
K = int(sys.argv[4])     # Number of eigenvectors/eigenvalues of SVD
...
N_TOP = int(N_PREDICT * SAMPLE_RATE)
...
# Evaluation.
pos2id = lambda p: (users[p[0]], repos[p[1]])   # Convert biadjacency matrix position (user_index, repo_index) to (user_id, repo_id)
print("\nEvaluating...")
correct_sinh = 0
for pred in top_sinh:
    if pos2id(pred[0]) in target_edges:
        correct_sinh += 1
correct_rr = 0
for pred in top_rr:
    if pos2id(pred[0]) in target_edges:
